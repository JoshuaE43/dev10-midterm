...
# Scatter Plot: median age vs total fertility rate
sctr_plt = px.scatter(
    df_merged,         
    x="median_age",
    y="tfr",
...
# Merged with countries table to add country names and regions
df_merged = pd.merge(df_merged, df_countries, on="alpha_2", how="left")

# A list of options for the country dropdown menu
country_options = []
for i, row in df_countries.iterrows(): # iterrows gives back tuple
    country_options.append({
        "label": row["name"] + " (" + row["region"] + ")", # appears on dropdown
        "value": row["name"] # what dash uses
    })

# Determine the dominant age group for each row by finding the max percentage column
...
# To ensure consistent display in plots
age_order = ['0-14', '15-19', '20-39', '40-64', '65+']

# Box Chart: fertility rate and dominant age group
box_chart = px.box(
    df_merged,
...
# Extract all unique years from the data for the slider
years = sorted(df_merged["year"].unique())
start_year = int(years[0])
end_year = int(years[-1])
default_year = start_year

# Labels for the slider ticks
marks_dict = {
...
# All columns treated as strings so sorting/filtering works
...
def update_chart(selected_countries, selected_year):
    ...
    # Filter data based on selected countries/year
